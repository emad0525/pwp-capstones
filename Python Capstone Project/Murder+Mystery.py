...
def prepare_text(text):
    revised_words = text.split()
    return [(w.rstrip(string.punctuation)).lower() for w in revised_words]
...
def ngram_creator(text_list):
    ...
    for i in range(len(text_list)):
        try:
            n_gram_list.append(text_list[i] + " " + text_list[i+1])
        except:
            IndexError
    return n_gram_list

print(ngram_creator(prepare_text(lily_trebuchet_intro)))
print(build_frequency_table(ngram_creator(prepare_text(lily_trebuchet_intro))))
...
def frequency_comparison(table1, table2):
    ...
    for x, y in table1.items():
        if x in table2:
            for j, k in table2.items():
                if x==j:
                    if y<k:
                        mutual_appearances += y
                        appearances += k
                    else:
                        mutual_appearances += k
                        appearances += y
        else:
            appearances += y
    for l, m in table2.items():
        if l not in table1:
            appearances += m
    return (mutual_appearances / appearances)

print(build_frequency_table(sorted(prepare_text(lily_trebuchet_intro))))
print(build_frequency_table(sorted(prepare_text(murder_note))))
...
def percent_difference(text1, text2):
    return (abs(get_average_sentence_length(text1) - get_average_sentence_length(text2)) / ((get_average_sentence_length(text1)+get_average_sentence_length(text2))/2))
...
def find_text_similarity(TextSample1, TextSample2):
    ...
    word_count_similarity = frequency_comparison(build_frequency_table(prepare_text(TextSample1)), build_frequency_table(prepare_text(TextSample2)))
    ngram_similarity = frequency_comparison(build_frequency_table(ngram_creator(prepare_text(TextSample1))), build_frequency_table(ngram_creator(prepare_text(TextSample2))))
...
print("For contestant, {author}, the similarity score to the murder letter is {score}".format(author=lily_sample.author, score=lily_similarity_score))
print("For contestant, {author}, the similarity score to the murder letter is {score}".format(author=myrtle_sample.author, score=myrtle_similarity_score))
print("For contestant, {author}, the similarity score to the murder letter is {score}".format(author=gregg_sample.author, score=gregg_similarity_score))
...
max_score = 0
killer = ""
for x,y in similarity_dict.items():
    if y > max_score:
        max_score = y
        killer = x
print(killer)    
...
class TextSample:
    ...
    def __repr__(self):
        return "The author\'s name is {author} and the average sentence length is {length}".format(author=self.author, length=self.average_sentence_length)
...
lily_similarity_score = find_text_similarity(murderer_sample.raw_text, lily_sample.raw_text)
myrtle_similarity_score = find_text_similarity(murderer_sample.raw_text, myrtle_sample.raw_text)
gregg_similarity_score = find_text_similarity(murderer_sample.raw_text, gregg_sample.raw_text)

similarity_dict = {}
similarity_dict[lily_sample.author] = lily_similarity_score
similarity_dict[gregg_sample.author] = gregg_similarity_score
similarity_dict[myrtle_sample.author] = myrtle_similarity_score
